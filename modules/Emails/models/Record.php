...
class Emails_Record_Model extends Vtiger_Record_Model
{
	...
	public function send($addToQueue = false)
	{
		...
		if($this->get('fromemail')){
			$mailer->from_email = $this->get('fromemail');
			$mailer->initialize();
		}
		...
		$toEmailInfo = $this->get('toemailinfo') ? array_filter((array)json_decode((string)$this->get('toemailinfo'))) : array();
		$emailsInfo = array();
			...
			foreach ($selectedEmails as $selectedEmail) {
				...
				if (!$status) {
					...
					if (!empty($folderName) && !empty($mailString)) {
						...
						if ($connector->mBox && !is_bool($connector->mBox)) {
